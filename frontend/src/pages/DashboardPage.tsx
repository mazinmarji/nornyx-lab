  ...
  return (
    <div className="page">
      ...
      <section className="dashboard-overview">
        ...
        <div className="dashboard-stat-grid"><article><span>Demonstrated</span><strong>{dashboard.concepts_mastered.length}</strong><small>concepts</small></article><article><span>Awaiting evidence</span><strong>{dashboard.concepts_pending_evidence.length}</strong><small>concepts</small></article><article><span>Review</span><strong>{dashboard.concepts_needing_review.length}</strong><small>concepts</small></article><article><span>Capstone</span><StatusBadge status={dashboard.capstone_status} /></article></div>
      </section>
      {dashboard.advanced_standing ? (
        <section className="advanced-standing" data-testid="dashboard-advanced-standing">
          <div className="section-heading compact-heading"><div><p className="eyebrow">Advanced competence</p><h2>{dashboard.advanced_standing.advanced_competence_demonstrated ? "Demonstrated" : "Not yet demonstrated"}</h2></div><StatusBadge status={dashboard.advanced_standing.advanced_competence_demonstrated ? "complete" : "in_progress"} /></div>
          <p className="muted">{dashboard.advanced_standing.note}</p>
        </section>
      ) : null}
      <section className="dashboard-columns">
        ...
        <div className="dashboard-panel"><div className="section-heading compact-heading"><div><p className="eyebrow">Adaptive review</p><h2>Concept signals</h2></div></div><p className="muted concept-note">Completing a lesson and demonstrating its concepts are recorded separately. A concept moves to “demonstrated” only when a check that tests it is passed.</p><h3>Demonstrated</h3><div className="concept-line">{dashboard.concepts_mastered.length ? dashboard.concepts_mastered.map((concept) => <span key={concept}>{concept}</span>) : <span>No mastery evidence yet</span>}</div><h3>Needs review</h3><div className="concept-line concept-review">{dashboard.concepts_needing_review.length ? dashboard.concepts_needing_review.map((concept) => <span key={concept}>{concept}</span>) : <span>No review signals yet</span>}</div><h3>Awaiting evidence</h3><div className="concept-line concept-pending" data-testid="concepts-pending-evidence">{dashboard.concepts_pending_evidence.length ? dashboard.concepts_pending_evidence.map((concept) => <span key={concept}>{concept}</span>) : <span>Every taught concept has evidence</span>}</div>{dashboard.concepts_requiring_redemonstration.length ? <><h3>Needs demonstrating again</h3><p className="muted concept-note">You did demonstrate these, but under an older definition of what they mean. That earlier work is still in your record; it no longer counts towards mastery until you show it again.</p><div className="concept-line concept-review" data-testid="concepts-requiring-redemonstration">{dashboard.concepts_requiring_redemonstration.map((concept) => <span key={concept}>{concept}</span>)}</div></> : null}</div>
      </section>
      <section className="dashboard-panel dashboard-feedback" data-testid="dashboard-feedback-link">
        <div className="section-heading compact-heading"><div><p className="eyebrow">Optional · has no effect on your results</p><h2>Tell us how the course is going</h2></div></div>
        <p className="muted">Your feedback is saved on this computer. It is kept separate from your learner record and never changes your score, your progress, or what the academy reports you have demonstrated.</p>
        <Link className="button button-secondary" to="/feedback">Give course feedback</Link>
      </section>
      {/* The dialog says exactly what is and is not cleared. Resetting progress
          deliberately leaves feedback alone — wanting to start the curriculum
          again is not the same as withdrawing what you said about it — and a
          learner should not have to discover that afterwards. */}
      {resetOpen ? <div className="dialog-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setResetOpen(false); }}><section ref={resetDialogRef} className="dialog" role="dialog" aria-modal="true" aria-labelledby="reset-title" aria-describedby="reset-description" tabIndex={-1}><p className="eyebrow">Destructive local action</p><h2 id="reset-title">Reset all academy progress?</h2><p id="reset-description">This clears executions, scores, concept signals, and capstone status from the local learner record. Export first if you need a copy. Your feedback is kept — delete it separately on the <Link to="/feedback">feedback page</Link> if you want it gone.</p><div className="button-row"><button type="button" className="button button-secondary" onClick={() => setResetOpen(false)}>Keep my progress</button><button type="button" className="button button-danger" disabled={busy} onClick={() => void confirmReset()}>{busy ? "Resetting…" : "Reset everything"}</button></div></section></div> : null}
    </div>
  );
}
